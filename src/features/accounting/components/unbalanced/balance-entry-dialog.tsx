        ...
        >
            {selectedEntry && (
                <FormProvider {...dialogMethods}>
                    <form onSubmit={onSubmitBalanceEntry} className="space-y-6 pt-2">
                        {/* 2-Column Grid: Left (Summary Info) vs Right (Multi-COA List) */}
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
                            {/* ── Left Column: Summary & Info ── */}
                            <div className="md:col-span-5 space-y-4">
                                <div className="p-4 bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-slate-200/70 dark:border-slate-700/60 space-y-3.5">
                                    ...
                                    <div className="space-y-1.5 text-xs">
                                        ...
                                        <div className="font-semibold text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-900 p-2.5 rounded-xl border border-slate-200/60 dark:border-slate-800">
                                    ...
                                    <div className="grid grid-cols-3 gap-2 text-xs pt-1">
                                        <div className="bg-white dark:bg-slate-900 p-2 rounded-xl border border-slate-200/60 dark:border-slate-800">
                                            <span className="text-[10px] text-slate-400 block font-medium">Debit</span>
                                            <span className="font-bold text-emerald-600 dark:text-emerald-400 tabular-nums">
                                                {Number(selectedEntry.debit) > 0 ? formatRupiah(Number(selectedEntry.debit)) : "-"}
                                            </span>
                                        </div>
                                        <div className="bg-white dark:bg-slate-900 p-2 rounded-xl border border-slate-200/60 dark:border-slate-800">
                                            <span className="text-[10px] text-slate-400 block font-medium">Kredit</span>
                                        ...
                                        <div className="bg-amber-50/70 dark:bg-amber-950/30 p-2 rounded-xl border border-amber-200/60 dark:border-amber-900/40">
                                            <span className="text-[10px] text-amber-700 dark:text-amber-400 block font-medium">Selisih</span>
                                    ...
                                    {selectedEntry.description && (
                                        <div className="text-xs text-slate-600 dark:text-slate-300 pt-2 border-t border-slate-200/40 dark:border-slate-700/40 space-y-1">
                                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">
                                                Keterangan / Referensi
                                            </span>
                                            <p className="leading-relaxed bg-white dark:bg-slate-900 p-2.5 rounded-xl border border-slate-200/60 dark:border-slate-800 text-xs">
                                                {selectedEntry.description}
                                            </p>
                                        </div>
                                    )}
                                ...
                                <div className="p-3.5 rounded-2xl bg-indigo-50/70 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-900/40 text-indigo-900 dark:text-indigo-200 flex items-start gap-2.5 text-xs">
                                    <IconInfoCircle className="w-4 h-4 text-indigo-600 dark:text-indigo-400 shrink-0 mt-0.5" />
                                    <p className="leading-relaxed text-[11px]">
                                        Sistem akan membuat jurnal penyeimbang (balancing entry) secara otomatis menggunakan akun COA dan alokasi nominal di sebelah kanan untuk menetralkan selisih sebesar <strong>{formatRupiah(selectedDiff)}</strong>.
                                    </p>
                                </div>
                            </div>
